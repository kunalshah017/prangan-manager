    ...
    <WorkspacePage>
      ...
      <Modal isOpen={addOpen} onClose={() => !createExpense.isPending && setAddOpen(false)} title="Add manual expense" className="max-h-[90dvh] overflow-y-auto rounded-b-none sm:rounded-lg">
        <form onSubmit={submitExpense} className="space-y-4">
          ...
          <label className="block"><span className="mb-1 block text-sm font-semibold">Notes <span className="font-normal text-muted-foreground">(optional)</span></span><textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} maxLength={1000} rows={3} className="w-full rounded-xl border bg-background p-3" /></label>
