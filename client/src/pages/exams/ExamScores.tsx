...
export default function ExamScores() {
  ...
  return (
    <WorkspacePage className="space-y-6">
      <WorkspacePageHeader title={exam.name} description={`${getAssessmentCycleLabel(exam.cycle)} · ${levelName(exam.semesterLevel, exam.level)} · ${exam.totalMaxMarks} marks`} action={<Link to={path} className="inline-flex min-h-11 items-center justify-center rounded-md border border-input bg-background px-4 text-sm font-medium hover:bg-accent">All exams</Link>} />
