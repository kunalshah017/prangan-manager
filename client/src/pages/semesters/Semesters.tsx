...
const Semesters = () => {
  ...
  const semesterList = (semesters || []).sort(
              ...
              <div className="p-6">
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex items-center text-xs text-muted-foreground">
                    ...
                    <span>
                      Updated{" "}
                      {new Date(semester.updatedAt).toLocaleDateString("en-GB")}
                    </span>
                  </div>
        ...
        {semesterList.length === 0 && (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <p className="text-sm text-muted-foreground">
              {isAdmin()
                ? `Get started by creating the first semester for ${
                    center?.name || "this center"
                  }`
                : "No semesters available to view"}
            </p>
            {isAdmin() && (
              <Link
                to={`/projects/${projectId}/centers/${centerId}/semesters/new`}
